...
libcosmic asks cosmic-config for the user's theme at run time, so
`just cosmic run` already paints frq in whatever accent and surfaces are set in COSMIC Settings.
...
def main() -> None:
    ...
    lines = [
        ...
        "",
        "  libcosmic asks cosmic-config for these at run time, so `just cosmic run`",
        "  follows COSMIC Settings as it changes. A phone has no cosmic-config, so",
        "  the APK carries them instead. That is the one real difference between",
        f'  the two, and it is why this file is in git."',
