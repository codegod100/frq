 ...
 *
 * <p>Compiled into the APK's {@code classes.dex}. Rust must resolve this class
 * via {@code Activity.getClassLoader().loadClass} — {@code FindClass} from a
 * native worker thread uses the system loader and cannot see APK classes.
 *
 * <p>The native half is {@code crates/jolt-moq/src/android_camera.rs} in
 * jolt-native, and it needs the JavaVM and the Activity handed to it first:
 * {@code libjoltapp.so}'s glue calls {@code joltmoq_android_init} before Jolt
 * starts. Without that, {@link #start} is never reached at all.
 */
public final class CameraCapture {
    private static final String TAG = "FrqCamera";
    ...
    static {
        // Required, not belt-and-braces. The runtime resolves a `native` method
        // against the libraries this class loader was told about through
        // System.loadLibrary — and libjoltmoq.so is not one of them: nothing
        // Java loads it. It arrives as a DT_NEEDED of libjoltapp.so, which
        // libvidya dlopens, and a library that reached the process that way is
        // invisible to JNI method binding. So it is named here, where the class
        // that declares the natives is initialised. Already-mapped is fine;
        // loadLibrary finds it and registers it rather than mapping it twice.
        try {
            System.loadLibrary("joltmoq");
        } catch (UnsatisfiedLinkError e) {
            // Nothing below can work after this: every frame and every state
            // change goes out through a native method.
            Log.e(TAG, "libjoltmoq.so not loadable; camera capture is dead", e);
        }
    }
    ...
    private static void startBackgroundThread() {
        stopBackgroundThread();
        backgroundThread = new HandlerThread("frq-camera");
