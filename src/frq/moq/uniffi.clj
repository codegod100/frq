...
;; The variant table lives in `frq.moq.raw`, generated beside the entry points.
;; It was hand-written here once and that was a mistake worth recording: the
;; variants are INSERTED into, not appended to — turning moq-ffi's audio and
;; video features on adds Audio and Video at 5 and 6 and shifts every later
;; variant down two. A stale table does not answer "unknown variant"; it
;; answers a confident, wrong name for a real one, and the message beside it
;; still reads correctly, so nothing looks broken.
...
(defn- decode-error
  ...
  (if (< len 8)
    ...
    (let [variant (be-u32 data 0)
          n       (be-u32 data 4)
          n       (min n (- len 8))]
      {:variant (get raw/moq-error-variants variant :moq/unknown)
